...
func TestMessages_GetMessages(t *testing.T) {
	...
	status := "CLICKED"
	sent := int64(1589563865697)
	url := "/messages?cursor=cursor-1&tags=tags-1"
	...
	t.Run("makes requests for message ID", func(t *testing.T) {
		client := courier.CreateClient("key", &server.URL)
		rsp, err := client.GetMessages(context.Background(), "cursor-1", "tags-1")
		assert.Nil(t, err)
		assert.Equal(t, requestMessageID, rsp.Results[0].ID)
		assert.Equal(t, sent, rsp.Results[0].Sent)
		assert.Equal(t, status, rsp.Results[0].Status)
	})
}
